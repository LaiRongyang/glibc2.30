...
static void unlink_chunk (mstate av, mchunkptr p)
{
  ...
  if (!in_smallbin_range (chunksize_nomask (p)) && p->fd_nextsize != NULL)
    {
      if (p->fd_nextsize->bk_nextsize != p     || p->bk_nextsize->fd_nextsize != p)    //从所在的large bin 双向链表中摘除掉这个节点  https://zhuanlan.zhihu.com/p/373555570
...
#define fastbin_index(sz) \
  ((((unsigned int) (sz)) >> (SIZE_SZ == 8 ? 4 : 3)) - 2)  //fast bin 最小两个字长？


/* The maximum fastbin request size we support */
#define MAX_FAST_SIZE     (80 * SIZE_SZ / 4)  20*8     20*4
...
#define NONCONTIGUOUS_BIT     (2U)   //?????????干嘛用的
...
struct malloc_state
{
  ...
  struct malloc_state *next_free;   //和next 有什么区别
...
_int_free (mstate av, mchunkptr p, int have_lock) /// mstate  和 have_lock 谁维护？线程
{
  ...
  */
  ...
  else if (!chunk_is_mmapped(p)) {  //如果非mmap
    ...
    */
    ...
    if ((unsigned long)(size) >= FASTBIN_CONSOLIDATION_THRESHOLD) {  //超过阈值所以合并？
      if (atomic_load_relaxed (&av->have_fastchunks))
	malloc_consolidate(av);
