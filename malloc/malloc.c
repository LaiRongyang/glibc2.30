...
* Vital statistics:
  ...
  Minimum allocated size: 4-byte ptrs:  16 bytes    (including 4 overhead)  8-byte ptrs:  24/32 bytes (including, 4/8 overhead)      ptmalloc2 中的 "Minimum allocated size" 是指在该内存分配器中，能够被分配的最小内存块的大小。具体来说，当程序请求内存时，ptmalloc2 会以这个最小分配单位来分配内存，即使请求的内存量比这个值小。
       ...
       The maximum overhead wastage(浪费) (i.e., number of extra bytes
  ...
  Maximum allocated size:  4-byte size_t: 2^32 minus(减) about two pages 8-byte size_t: 2^64 minus about two pages
...
#include <malloc/malloc-internal.h> //malloc的内部声明，用于libc中。
...
libc_hidden_proto(__libc_malloc) // libc_hidden_proto 宏的作用是在共享库的头文件中声明一个函数原型，并将该函数标记为隐藏，从而限制它的可见性
...
#ifndef M_MXFAST // M_MXFAST 是一个在 ptmalloc2（glibc 中的内存分配器）中的宏，用于定义 fastbins（快速分配块）中内存块的最大大小。fastbins 是用于存储小块内存的链表，以便在需要分配小块内存时，可以快速地从 fastbins 中取出内存块，而无需进行内存分配器的其他复杂操作。
#define M_MXFAST 1
#endif
...
#define M_TRIM_THRESHOLD -1 // M_TRIM_THRESHOLD 是一个在 ptmalloc2（glibc 中的内存分配器）中的宏，用于指定在释放内存时触发 malloc_trim 函数的阈值。malloc_trim 函数可以将内存分配器的内部碎片（即未被分配的小块内存）返回给操作系统，以便系统更好地利用内存。
...
#define M_TOP_PAD -2 // M_TOP_PAD 是在 ptmalloc2（glibc 中的内存分配器）中的一个宏，用于定义堆顶（heap top）和堆底（heap bottom）之间的空闲空间。ptmalloc2 内存分配器在堆的顶部（high end）和底部（low end）保留了一些空闲空间，以便进行内存分配和释放时的处理。这个宏用于指定堆顶和堆底之间的保留空间的大小。
...
#ifndef DEFAULT_MMAP_THRESHOLD_MIN              // DEFAULT_MMAP_THRESHOLD_MIN 宏定义了一个内存分配请求的阈值。当程序请求的内存大小超过 DEFAULT_MMAP_THRESHOLD_MIN 时，ptmalloc2 就会选择使用 mmap 系统调用来分配内存。相反，如果请求的大小在 DEFAULT_MMAP_THRESHOLD_MIN 以下，ptmalloc2 通常会使用 brk 或 sbrk 来分配内存。
...
#else
#define DEFAULT_MMAP_THRESHOLD_MAX (4 * 1024 * 1024 * sizeof(long)) // 32MB  64位机
#endif
#endif
...
#define M_MMAP_THRESHOLD -3 // 当程序请求的内存大小超过 M_MMAP_THRESHOLD 时，ptmalloc2 就会选择使用 mmap 系统调用来分配内存，而不是使用 brk 或 sbrk 来扩展堆的大小。
...
struct malloc_chunk
{
  ...
  struct malloc_chunk *fd_nextsize; /* double links -- used only if free. */ // malloc_chunk 类型
  struct malloc_chunk *bk_nextsize;                                          //  fd_nextsize  bk_nextsize 构建的数据结构是 一个large bin 由多个 chunk 链表 组成
};
...
nextchunk-> +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    ...
    to extend or adapt this code.  这使得处理对齐等更加容易，但在尝试扩展或调整此代码时可能会非常混乱。？？？？？？？
  ...
  field. 如果设置了M位，则忽略其他位
  (因为mmapped块既不在arena中，也不与释放的chunk相邻）.
   The M bit is also used for chunks which
  ...
  从区块分配器的角度来看，fastbins中的chunks 被视为已分配的chunks它们仅与邻居进行批量合并，在 malloc_consolidate.
  ？？？？？？？？？？？？？？？
*/
...
#define MIN_CHUNK_SIZE (offsetof(struct malloc_chunk, fd_nextsize)) // 两个size 和两个指针

/*我们可以malloc的最小大小是对齐的最小chunk */

#define MINSIZE \
  (unsigned long)(((MIN_CHUNK_SIZE + MALLOC_ALIGN_MASK) & ~MALLOC_ALIGN_MASK)) //???????????
// 00000100000+00000011111  & 11111110000  =   00000100000    4个字长
...
#define chunk_at_offset(p, s) ((mchunkptr)(((char *)(p)) + (s))) // ？？？？？？？？？如果你有一个大的数据结构或者内存块，你可以使用 "chunk_at_offset" 来表示在特定偏移量处的子块数据。

/* extract p's inuse bit */
#define inuse(p) \
  ((((mchunkptr)(((char *)(p)) + chunksize(p)))->mchunk_size) & PREV_INUSE) // 从下一个chunk获取
...
/*
  ...
  所有内部状态都保存在下面定义的malloc_state实例中。除两种可选情况外，没有其他静态变量：
...
/*
   Bins
    ...
    An array of bin headers for free chunks. Each bin is doubly  空闲chunks的bin头数组。每个bin都是双向链接的。bins的空间大致成比例（对数）。
...
#define bin_at(m, i) \ //根据malloc_state指针  获取第i 个bin的指针，指针是 malloc_chunk * 类型
(mbinptr)(((char *)&((m)->bins[((i)-1) * 2])) - offsetof(struct malloc_chunk, fd))

/* analog of ++bin */ //   b   是  malloc_chunk 类型
#define next_bin(b) ((mbinptr)((char *)(b) + (sizeof(mchunkptr) << 1)))

/* Reminders（提醒） about list directionality（方向） within bins */
#define first(b) ((b)->fd)
#define last(b) ((b)->bk)
...
/*
   Indexing
    ...
    32 bins of size      64
    16 bins of size     512
     8 bins of size    4096    4  kB
     4 bins of size   32768   32  kB
     2 bins of size  262144  256  kB
     1 bin  of size what's left     1个剩余大小的bin
    127

    There is actually a little bit of slop in the numbers in bin_index
    for the sake of speed. This makes no difference elsewhere.???

    bins 最高可达1MB左右，因为我们希望通过mmap为大型请求提供服务
...
#define SMALLBIN_CORRECTION (MALLOC_ALIGNMENT > 2 * SIZE_SZ)                 // CORRECTION（校正）
#define MIN_LARGE_SIZE ((NSMALLBINS - SMALLBIN_CORRECTION) * SMALLBIN_WIDTH) // 64×16  4k
    ...
    static void unlink_chunk(mstate av, mchunkptr p)
{
  ...
  if (__builtin_expect(fd->bk != p || bk->fd != p, 0)) //__builtin_expect 这个函数的作用是为了告诉编译器条件的预期结果，以便它能够更好地进行指令重排（instruction reordering）和其他性能优化，从而提高程序的执行效率。
    malloc_printerr("corrupted double-linked list");

  fd->bk = bk; // 从同一size的chunk链删除当前chunk
  bk->fd = fd;
  //// 从所在的large bin 双向链表中摘除掉这个节点  https://zhuanlan.zhihu.com/p/373555570
  if (!in_smallbin_range(chunksize_nomask(p)) && p->fd_nextsize != NULL) // 如果是large chunk，并且有相邻的chunk链
  {
    if (p->fd_nextsize->bk_nextsize != p || p->bk_nextsize->fd_nextsize != p)
      malloc_printerr("corrupted double-linked list (not small)");

    if (fd->fd_nextsize == NULL) // 如果fd的fd_nextsize字段为空,（表示当前chunk链不止一个chunk块）
    {
      if (p->fd_nextsize == p)                  // 如果只有一个chunk链
        fd->fd_nextsize = fd->bk_nextsize = fd; // 更新fd的fd_nextsize bk_nextsize字段为它自己
      else
      {
        fd->fd_nextsize = p->fd_nextsize; // 将前一个 当前一个 后一个chunk链 重新链接
        fd->bk_nextsize = p->bk_nextsize;
        p->fd_nextsize->bk_nextsize = fd;
        p->bk_nextsize->fd_nextsize = fd;
      }
    }
    else // fd的fd_nextsize字段不为空,（表示当前chunk链只有 一个chunk块）
    {
...
/*
   Unsorted chunks
    ...
    and taken off (to be either used or placed in bins) in malloc.
      ...
      来自区块分割的所有剩余部分，以及所有返回的区块，都会首先放在“未排序”的bin中。在malloc给它们一次使用的机会后，它们被放在普通的垃圾箱中。
    因此，基本上，unsorted_counds列表充当一个队列，其中的块以free（和malloc_complate）的形式放在上面，以malloc的形式取下（使用或放在bin中）。
...
#define unsorted_chunks(M) (bin_at(M, 1)) // 也就是 bin[0] 和 bin [1]是不用的
...
/* Conveniently, the unsorted bin can be used as dummy top on first call */ // 方便的是，未排序的bin可以在第一次调用时用作伪top
...
/*
   Binmap
    ...
    be skipped over during during traversals.（遍历）  The bits are NOT always
...
#define BINMAPSHIFT 5                   //  在堆管理中，通常使用位图来跟踪哪些内存块是空闲的，哪些是被分配的。BINMAPSHIFT 定义了位图中每个字节的位数。
                                        //  例如，如果 BINMAPSHIFT 被设置为5，那么每个字节将包含32个位，表示32个相邻的内存块的状态（空闲或已分配）。
#define BITSPERMAP (1U << BINMAPSHIFT)  // 表示每个位图元素（通常是一个字节）中的位数。在上面的例子中，BITSPERMAP 的值就是32，表示每个字节有32个位。
#define BINMAPSIZE (NBINS / BITSPERMAP) // 表示堆管理中位图的大小（以字节数为单位）。位图的大小通常是由空闲块的数量和 BINMAPSHIFT 的值来计算的，以便能够表示所有的空闲块的状态。

#define idx2block(i) ((i) >> BINMAPSHIFT)                      // 根据index找到 Binmap 中对应的block
#define idx2bit(i) ((1U << ((i) & ((1U << BINMAPSHIFT) - 1)))) // 根据index找到 找到所属block中对应的bit
...
/*
   Fastbins
    ...
    fastbin 和 unsorted bin 是堆管理算法中两种不同类型的空闲内存块链表，用于管理不同大小的空闲内存块。
    fastbin 用于小内存块（通常小于128字节）的分配。
    unsorted bin 用于中等大小的内存块（大于128字节，但小于 fastbin 阈值）。
    fastbin 中的块通常是按大小顺序排列的，以方便快速分配。
    unsorted bin 中的块通常是未排序的。
 */
...
#define fastbin_index(sz) ((((unsigned int)(sz)) >> (SIZE_SZ == 8 ? 4 : 3)) - 2) // fast bin 最小两个字长？

/* The maximum fastbin request size we support */
#define MAX_FAST_SIZE (80 * SIZE_SZ / 4) // 20 * 8
...
/*
   ...
   FASTBIN_CONSOLIDATION_THRESHOLD是free（）中的一个块的大小，
   它触发可能周围的FASTBIN块的自动合并。这是一种启发式方法，
   因此确切的值应该不会太重要。它被定义为默认修剪阈值的一半，
   作为一种折衷启发式方法，只有在可能导致修剪的情况下才尝试合并。
   然而，它不是动态可调的，因为即使不使用修剪，合并也会减少大块周围的碎片。
 */
...
/*
   ...
   NONCONTIGOUS_BIT表示MORECORE不返回连续区域。否则，在可能的情况下，会利用连续MORECORE调用的结果进行合并。
  初始值来自MORECOE_CONTIGUOUS，但如果将mmap用作sbrk替代，则会动态更改。
 */
//?????????干嘛用的
...
/*
   ...
   have_fastchunks表示可能存在一些fastbin块。在任何fastbin中输入一个chunk时都设置为true，并在malloc_consolidate中提前清除。
   该值是近似值，因为它可以在没有fastbin块的情况下设置，或者即使有可用的fastbin块，它也可能是清楚的。
   考虑到它的唯一目的是减少对malloc_consolidate的冗余调用数量，
   它不会影响正确性。因此，我们可以安全地使用宽松的原子访问。
 */

struct malloc_state
{
  /* Serialize access.  */
  __libc_lock_define(, mutex); // 多线程？
...
#define DUMPED_MAIN_ARENA_CHUNK(p) ((p) >= dumped_main_arena_start && (p) < dumped_main_arena_end)
...
/*
   ...
   This is called from ptmalloc_init () or from _int_new_arena () when creating a new arena.
 */
  ...
  { /* Request enough space for nb + pad + overhead */
    ...
    if (brk != (char *)(MORECORE_FAILURE))
    {
      ...
      /*
         ...
         如果MORECORE扩展先前的空间，我们同样可以扩展顶部大小。
       */
...
/*
   ...
   systrim与sysmalloc相反。
   如果malloc池的“高”端有未使用的内存，它会将内存返回给系统（通过sbrk的负参数）。
   当顶部空间超过修剪阈值时，free（）会自动调用它。
   它也由公共malloc_trim例程调用。如果它实际释放了任何内存，则返回1，否则返回0。
 */
...
systrim(size_t pad, mstate av)
{
  ...
  top_size = chunksize(av->top); // top chunk 大小
  ...
  extra = ALIGN_DOWN(top_area - pad, pagesize); // 以页面大小向下对齐
...
_int_malloc(mstate av, size_t bytes)
{
  ...
  /* There are no usable arenas.  Fall back to sysmalloc to get a chunk from
     mmap.  */
  if (__glibc_unlikely(av == NULL)) //__glibc_unlikely 宏的作用是告诉编译器某个条件语句不太可能为真，这样编译器在生成汇编代码时，可以将执行路径的优先级调整为不太可能的分支。
  {
    void *p = sysmalloc(nb, av); // 这个chunk 属于哪个arena?????
    if (p != NULL)
...
#define REMOVE_FB(fb, victim, pp) \
  ...
  if ((unsigned long)(nb) <= (unsigned long)(get_max_fast()))
  {
    ...
    if (victim != NULL)
    {
      if (SINGLE_THREAD_P) //  /* 没有启用多线程的情况 */
        *fb = victim->fd;  // 更新bin指针
      else
        REMOVE_FB(fb, pp, victim); // 线程安全地操作
  ...
  /*
     If a small request, check regular bin.  Since these "smallbins"  如果有小的要求，请检查常规垃圾箱。由于这些“小箱子”每个都有一个尺寸，因此无需在箱子内进行搜索。
  ...
  /*
     如果这是一个较大的请求，请在继续之前合并fastbin。
     虽然在查看是否有可用空间之前杀死所有fastbin可能看起来有些过分，但这避免了通常与fastbin相关的碎片问题。
     此外，在实践中，程序往往会运行小请求或大请求，但混合请求的情况较少，因此在大多数程序中并不会经常调用合并。
     And the programs that it is called frequently in otherwise tend to fragment.？？？？
   */
  ...
  /*
     ...
     chunks are placed in bins.
      处理最近释放或剩余的块，只在完全匹配的情况下获取一个，
      或者，如果这是一个小请求，则chunks是最近非完全匹配的剩余chunks。
      将其他已遍历的chunks放入bins中。请注意，在任何例程中，这一步骤都是将块放置在bin中的唯一位置。
     ...
     这里需要外循环，因为我们可能直到malloc接近尾声时才意识到我们应该合并，所以必须这样做并重试。
     这种情况最多发生一次，而且只有当我们需要扩展内存来为“小”请求提供服务时才会发生。
   */
  ...
  {
    int iters = 0;
    while ((victim = unsorted_chunks(av)->bk) != unsorted_chunks(av)) // 遍历 unsorted bin的双向链表，如果 只有一个chunk且比需求大，则切开返回，
                                                                      // 如果刚好和需求一样也摘除返回，
                                                                      // 其他情况摘掉 放回到对应的small bin 和large bin中
                                                                      // 遍历迭代次数不超过 MAX_ITERS       10000
    {
      bck = victim->bk;
      ...
      if (in_smallbin_range(nb) &&                               // 如果in small range
          bck == unsorted_chunks(av) &&                          // unsorted_chunks有唯一空闲块
          victim == av->last_remainder &&                        // unsorted_chunks有空闲块的空闲块是last_remainder
          (unsigned long)(size) > (unsigned long)(nb + MINSIZE)) // 空闲块的size > nb + MINSIZE ？？？？？
      {
        ...
        set_head(remainder, remainder_size | PREV_INUSE);
        set_foot(remainder, remainder_size); // 设置这个remainder
      ...
      unsorted_chunks(av)->bk = bck; // 非unsorted_chunks的唯一空闲块 摘除这个空闲块
      bck->fd = unsorted_chunks(av);

      /* Take now instead of binning if exact fit */

      if (size == nb) // 如果空闲块刚好合适
      {
     ...
     We may return one of these chunks later.  */
      ...
      if (in_smallbin_range(size))
      {
        ...
        fwd = bck->fd; // 获取这个bin的前向第一个chunk ??    //  mchunkptr fwd;                    /* misc temp for linking */
      }
      else
      {
        victim_index = largebin_index(size);
        bck = bin_at(av, victim_index); // 定位到对应的largeb bin
        fwd = bck->fd;

        /* maintain large bins in sorted order */
        if (fwd != bck) // large bins有空闲块
        {
          /* Or with inuse bit to speed comparisons */ //????????
          size |= PREV_INUSE;
          /* if smaller than smallest, bypass loop below */
          assert(chunk_main_arena(bck->bk));
          if ((unsigned long)(size) < (unsigned long)chunksize_nomask(bck->bk)) // 如果size 比后向第一个还小
          {
            fwd = bck;
            bck = bck->bk;
            ...
            fwd->fd->bk_nextsize = victim->bk_nextsize->fd_nextsize = victim; // ？？？？？？？？？？？？？？？将空闲块victim放到该bin
          }
          else
          {
            assert(chunk_main_arena(fwd));
            while ((unsigned long)size < chunksize_nomask(fwd)) // 前向寻找这个large bin中第一个chunk链表，这个fwd对应的链表的第一个节点比这个unsorted bin 的chunk的size要小或者相等
            {
              fwd = fwd->fd_nextsize;
              assert(chunk_main_arena(fwd));
            }
            ...
            if ((unsigned long)size == (unsigned long)chunksize_nomask(fwd))
              ...
              fwd = fwd->fd; // 查到这个链表的第二个位置
            else
            { // fd_nextsize  bk_nextsize 串起来的 双向链表 按前向的顺序是从大到小的 双向链表的的节点又是一个fd bk连接的双向链表的头节点，这个双向链表每个节点大小都一样
          ...
          victim->fd_nextsize = victim->bk_nextsize = victim; // 插入到large bin??
      }

      mark_bin(av, victim_index); // 标记一下这个bin有东西？？？
      victim->bk = bck;           // unsorted bin 的victim插入到双向链表中
      victim->fd = fwd;
      fwd->bk = victim;
      bck->fd = victim;
...
#define MAX_ITERS 10000
    ...
    /*
       ...
       如果是一个大的请求，请按排序顺序扫描当前bin的块，以找到适合的最小块。请使用跳过列表？？？。
     */

    if (!in_smallbin_range(nb)) // 非小请求
    {
      bin = bin_at(av, idx); // 找到对应的large bin

      /* skip scan if empty or largest chunk is too small */
      if ((victim = first(bin)) != bin                                       // 这个bin有东西
          && (unsigned long)chunksize_nomask(victim) >= (unsigned long)(nb)) // 前向第一个chunk大小满足需求
      {
        victim = victim->bk_nextsize;                                             //????这方向是不是反了
        while (((unsigned long)(size = chunksize(victim)) < (unsigned long)(nb))) // 找到第一个大于等于需求的 large chunk 链
          victim = victim->bk_nextsize;

        /* Avoid removing the first entry for a size so that the skip list does not have to be rerouted.  */
        if (victim != last(bin)                                          // 不是最后一个双向链表
            && chunksize_nomask(victim) == chunksize_nomask(victim->fd)) // 第一第二个节点大小一样
          victim = victim->fd;                                           // 指向第二个节点

        remainder_size = size - nb; // 计算多余空间
        unlink_chunk(av, victim);   // 摘除节点

        /* Exhaust */
        if (remainder_size < MINSIZE) // 如果剩余空间少于minsize，就不切割
        {
          set_inuse_bit_at_offset(victim, size);
          if (av != &main_arena)
            set_non_main_arena(victim);
        ...
        else // 切割  插到unsorted chunk
        {
          ...
          bck->fd = remainder;
          fwd->bk = remainder;
          if (!in_smallbin_range(remainder_size)) // 如果remainder是large chunk  设置 fd_nextsize bk_nextsize
          {
            remainder->fd_nextsize = NULL;
            remainder->bk_nextsize = NULL;
          }
    ...
    /*
       ...
       that fits is selected.
        通过扫描垃圾箱来搜索区块，从下一个最大的垃圾箱开始。此搜索严格按照最佳匹配进行；即选择适合的最小（平局接近于最近使用最少的）块。

       The bitmap avoids needing to check that most blocks are nonempty.
       The particular case of skipping all bins during warm-up phases
       when no chunks have been returned yet is faster than it might look.

       位图避免了检查大多数块是否为空。在预热阶段跳过所有存储箱的特殊情况下，当还没有返回块时，速度比看起来更快。
     */

    ++idx; // idx 是需求对应的bin index 但是为什么要++  是不是当前bin不能满足就往下一个bin寻找
    bin = bin_at(av, idx);
    block = idx2block(idx);
    map = av->binmap[block];
    bit = idx2bit(idx);
    ...
    for (;;) //
    {
      ...
      if (bit > map || bit == 0) // 这个bit已经超出这个block
      {
        do
        {
          if (++block >= BINMAPSIZE) /* out of bins */ // 下一个block   ,当前block是不是最后一个block了
            goto use_top;                              //??????????????
        } while ((map = av->binmap[block]) == 0);      // 找到有空闲块的bin的block
      ...
      while ((bit & map) == 0) // 找到这个map中有效的一个bit???
      {
        bin = next_bin(bin);
        bit <<= 1;
        assert(bit != 0);
      ...
      victim = last(bin); // 反向第一个chunk

      /*  If a false alarm (empty bin), clear the bit. */
      if (victim == bin) // empty bin
      {
      ...
      else
      {
        ...
        {
          remainder = chunk_at_offset(victim, nb); // 如果剩余空间大于minisize就切割
          ...
          remainder->fd = fwd;
          bck->fd = remainder;
          fwd->bk = remainder; // 插到unsorted bin

          /* advertise as last remainder */
          if (in_smallbin_range(nb)) // 如果是samll chunk 就   更新 last_remainder
            av->last_remainder = remainder;
          if (!in_smallbin_range(remainder_size)) // large chunk 就设置指针
          {
            remainder->fd_nextsize = NULL;
            remainder->bk_nextsize = NULL;
          }
  ...
  use_top:
    ...
    if ((unsigned long)(size) >= (unsigned long)(nb + MINSIZE)) // 如果堆顶的空闲区域满足需求就分配该区域
    {
      remainder_size = size - nb;
      remainder = chunk_at_offset(victim, nb);
      av->top = remainder;
...
static void
_int_free(mstate av, mchunkptr p, int have_lock) /// mstate  和 have_lock 谁维护？线程
{
  INTERNAL_SIZE_T size;     /* its size */
  ...
  /*
    If eligible（符合条件的）, place chunk on a fastbin so it can be found
    and used quickly in malloc.
  */

  if ((unsigned long)(size) <= (unsigned long)(get_max_fast()) // 如果大小符合fast bin
  ...
  )
  {
    ...
    fb = &fastbin(av, idx); // 找到对应的fast bin
  ...
  */
  ...
  { // 如果非mmap
    ...
    nextchunk = chunk_at_offset(p, size); // next chunk地址
    ...
    nextsize = chunksize(nextchunk); // next chunk 大小
    ...
    if (!prev_inuse(p))
    {
      ...
      size += prevsize; // size 合并
      p = chunk_at_offset(p, -((long)prevsize));
      if (__glibc_unlikely(chunksize(p) != prevsize))
        malloc_printerr("corrupted size vs. prev_size while consolidating");
      unlink_chunk(av, p); // 将后向的chunk 摘除，并将指针指向后向的那个chunk
    }
    ...
    if (nextchunk != av->top)
    {
      ...
      if (!nextinuse)
      {                              // 如果前向的chunk空闲
        unlink_chunk(av, nextchunk); // 合并前向的空闲块
        size += nextsize;
      }
      else
        clear_inuse_bit_at_offset(nextchunk, 0); // 否则设置一下前向的chunk的inuse
      ...
      */
      ...
      p->fd = fwd; // 将空闲块插到unsorted bin
      p->bk = bck;
      if (!in_smallbin_range(size)) // 大chunk
      {
        p->fd_nextsize = NULL;
        p->bk_nextsize = NULL;
      }
